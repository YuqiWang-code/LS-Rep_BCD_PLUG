...
class BaseNet_LWGANet_L0(nn.Module):
    def __init__(self, pretrained=True, use_afd=False):
        ...
        # AFD modules (only for training)
        if use_afd:
            # LWGANet L0 feature channels: [32, 32, 64, 128, 256]
            # After SWA aggregation: all become mid_d = 64
    ...
    def forward(self, x1, x2):
        ...
        # AFD loss calculation (only during training)
        ...
        if self.training and self.use_afd:
            return (mask_p2, mask_p3, mask_p4, mask_p5), loss_afd
        return mask_p2, mask_p3, mask_p4, mask_p5
...
class BaseNet_LWGANet_L2(nn.Module):
    def __init__(self, pretrained=True, use_afd=False):
        ...
        # AFD modules (only for training)
        if use_afd:
            # LWGANet L2 feature channels: [96, 96, 192, 384, 768]
            # After SWA aggregation: all become mid_d = 64
            self.AFD_semantic_4 = AFD_semantic(self.mid_d, 0.0625)
            self.AFD_semantic_3 = AFD_semantic(self.mid_d, 0.0625)
            self.AFD_spatial_2 = AFD_spatial(self.mid_d)
