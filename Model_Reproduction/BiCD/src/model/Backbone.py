...
from src.model.mobilenetv2 import MobileNetV2
from src.model.swin_transformer import SwinTransformer


# Get corresponding model based on model name
...
# Get channel numbers for each layer of the model
...
# Multi-Task Feature Module (MTF)
class MTF(nn.Module):
    def __init__(self, channel, mode='iade', kernel_size=1):
        super(MTF, self).__init__()
        assert mode in ['i', 'a', 'd', 'e', 'ia', 'id', 'ie', 'iae', 'ide', 'iad', 'iade', 'i2ade', 'iad2e', 'i2ad2e',
                        'i2d']
        self.mode = mode
...
# Multi-Scale Feature Module (MSF)
class MSF(nn.Module):
    ...
    def forward(self, t0_fs, t1_fs=None):
        ...
        final_list = [F.interpolate(cf_layer, resize_shape, mode='nearest') for cf_layer in final_list]
        cf = torch.cat(final_list, dim=1)
        cf = self.relu(self.bn(self.reduce(cf)))
        return cf


# FPN-based model with single backbone
...
# Dual backbone with MTF/MSF (currently focusing on this)
...
# Alternative dual backbone with MTF/MSF
...
# Create FPN-based model with single backbone
...
# Create MTF-based backbone model
...
# Create dual backbone with MTF/MSF (currently focusing on this)
...
# Create alternative dual backbone with MTF/MSF
