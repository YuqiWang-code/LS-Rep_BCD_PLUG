...
class ConfuseMatrixMeter(AverageMeter):
    """Computes and stores confusion matrix metrics"""
    ...
    def update_cm(self, pr, gt, weight=1):
        """Update confusion matrix and return current F1 score"""
...
def compute_metrics_accumulated(tp, tn, fp, fn):
    """
    Compute metrics from accumulated confusion matrix values
    Compatible with LS-Rep_BCD metrics format
    """
    ...
    # Kappa
    pre = ((tp + fn) * (tp + fp) + (tn + fp) * (tn + fn)) / ((tp + fp + tn + fn) ** 2 + eps)
