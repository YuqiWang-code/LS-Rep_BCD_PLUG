...
【研究定位与原则】
本项目是长期学术研究项目（以学术创新为主，非工程化/生产项目）：
1. 以学术创新为主：创新点应落在外挂模块的结构、机制或学习范式上，代码为实验服务，不追求生产级工程化。
2. 不把损失函数（loss）本身作为创新点：loss 只是训练工具，不是贡献点。
3. 实验以证明方法有效性为主（四个数据集一致提升），不追求吞吐/显存/推理速度等工程指标。
4. 长期迭代：项目会不断更新版本、修改代码；给方案时优先考虑可维护、可扩展、便于消融与版本回溯，避免把一次性 hack 写死进主结构。
